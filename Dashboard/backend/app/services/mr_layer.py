...
# The street network drawn by this app is required for every flow.
STREETS = DASHBOARD.parent / "media" / "street-network.geojson"
...
# Its flow is rejected rather than drawn through buildings.
SNAP_LIMIT_M = 250.0
        ...
        for dx in range(-radius, radius + 1):
            for dy in range(-radius, radius + 1):
                ...
                for index in graph["buckets"].get((cx + dx, cy + dy), ()):
                    # A lone survey point has no street segment to travel on.
                    # AWL has one closer than the connected road beside it.
                    if not graph["adjacency"][index]:
                        continue
...
def build_layer(dispatch: dict, placements: dict | None = None) -> dict:
    ...
    for link in dispatch["links"]:
        ...
        # Every visible connection must follow the campus street network.
        # A missing or disconnected network is an error rather than a different
        # drawing of the same community.
        route = street_route(a, b, obstacles)
        if route is None:
            if not STREETS.is_file():
                raise ValueError(f"Street network is missing: {STREETS}")
            raise ValueError(
                f"No street route for {source} -> {target}; check the node "
                f"positions and {STREETS}"
            )
