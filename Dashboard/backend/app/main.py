...
# Keep the known campus PV profiles in Git while leaving newly fetched cache
# entries local. The layer can then dispatch without a PVGIS network request.
PVGIS_CACHE = CACHE_ROOT / "pvgis"
PVGIS_CACHE.mkdir(parents=True, exist_ok=True)
for profile in (BACKEND_ROOT / "data" / "pvgis_profiles").glob("*.json"):
    cached = PVGIS_CACHE / profile.name
    if not cached.exists():
        shutil.copyfile(profile, cached)

pvgis = PVGISCache(cache_dir=PVGIS_CACHE)
...
app.add_middleware(
    ...
    allow_origin_regex=r"http://(localhost|127\.0\.0\.1):(51[7-9][0-9]|55[0-9][0-9])",
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def local_demand_paths(definition: dict) -> dict:
    """Resolve portable scenario CSV paths against this app's root."""
    for building in definition.get("buildings", []):
        demand = building.get("demand") or {}
...
def get_scenario(name: str) -> dict:
    ...
    definition = local_demand_paths(json.loads(path.read_text(encoding="utf-8")))
