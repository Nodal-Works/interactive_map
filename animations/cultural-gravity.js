...
(function () {
  'use strict';
  if (!window.APP_CONFIG?.culturalSites?.length) return;
  ...
  const CULTURAL_SITES = window.APP_CONFIG?.culturalSites || [];
  ...
  channel.addEventListener('message', ({data})=>{if(data.type==='cultural_gravity_control'){if(data.action==='next')advanceSequence();if(data.action==='stop'){stop();channel.postMessage({type:'animation_state',animationId:'cultural-gravity-btn',isActive:false});document.getElementById('cultural-gravity-btn')?.classList.remove('active');}}});
