...
def terrain_without_barriers(dem: np.ndarray, buildings: np.ndarray) -> np.ndarray:
    """Restore approximate ground below legacy stormwater barriers from nearby ground."""
    ground = np.isfinite(dem) & ~buildings
    if not ground.any():
        raise ValueError("Local terrain has no valid ground outside buildings")
    nearest = distance_transform_edt(~ground, return_distances=False, return_indices=True)
    result = dem.copy()
    result[buildings] = dem[tuple(nearest)][buildings]
    return result


def local_ground_raster(affine, shape) -> np.ndarray:
    # Lindholmen's legacy asset generator burned artificial building barriers into
    # this terrain file. Remove those before adding thermal building heights.
    path = ROOT / "media/clipped_dem.geotiff.tif"
    footprints = json.loads((ROOT / "media/building-footprints.geojson").read_text())
    ...
    with rasterio.open(path) as src:
        ...
        buildings = rasterize(((shape_transform(projector, geometry_shape(f["geometry"])), 1)
                              for f in footprints["features"]), out_shape=src.shape,
                             transform=src.transform, fill=0).astype(bool)
        ground = terrain_without_barriers(dem, buildings)
        result = np.full(shape, np.nan, dtype=np.float32)
        reproject(source=ground, destination=result, src_transform=src.transform,
                  src_crs=src.crs or CRS, src_nodata=np.nan, dst_transform=affine,
...
def prepare(force: bool = False) -> Path:
    ...
    if manifest.exists() and json.loads(manifest.read_text()).get("location") != "lindholmen":
        raise RuntimeError("This data directory contains another location; choose a separate COOLPATHS_DATA_DIR for Lindholmen")
    ...
    local_dem = local_ground_raster(affine, shape)
    ...
    local_water = aligned_raster(ROOT / "media/water_mask.tif", affine, shape,
                                 Resampling.nearest, source_crs=CRS)
    water = np.where(np.isfinite(local_water), local_water > 0.5, water)
    ...
    metadata = {
        ...
        "location": "lindholmen",
        "local_terrain_processing": "Legacy building barriers replaced by nearest valid ground before adding building heights; local water mask retained",
