...
(function () {
  ...
  try {
    const selected = localStorage.getItem('interactive_map_selected_calibration');
    const history = JSON.parse(localStorage.getItem('interactive_map_calibrations') || '[]');
    const saved = history.find(c => c.id === selected) || JSON.parse(localStorage.getItem('interactive_map_default_calibration') || 'null');
