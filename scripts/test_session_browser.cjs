...
(async()=>{
  ...
  await phone.getByRole('button',{name:'Show all',exact:true}).click();
  await page.waitForFunction(()=>reveal?.index===reveal?.values.length-1);
  await phone.getByRole('button',{name:'Auto reveal',exact:true}).click();
  await phone.getByRole('button',{name:'Pause reveal',exact:true}).click();
  ...
  await phone.getByRole('button',{name:'Next slide',exact:true}).click();
  await page.waitForFunction(()=>currentSlideIndex===1 && slideStatus==='ready');
