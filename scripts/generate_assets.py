...
def wgs84_to_sweref(west, south, east, north):
    ...
    return TO_SWEREF.transform_bounds(west, south, east, north)


def sweref_to_wgs84(xmin, ymin, xmax, ymax):
    """Convert a SWEREF99 TM bounding box to WGS84."""
    return TO_WGS84.transform_bounds(xmin, ymin, xmax, ymax)
...
def generate_footprints(bounds, output_dir, wgs84_bbox):
    ...
    try:
        ...
        raise RuntimeError("geopandas is required to export WGS84 footprints")
        # Legacy fallback is intentionally unreachable
        out_path = output_dir / "building-footprints.geojson"
...
def generate_water_bodies(bounds, output_dir):
    ...
    try:
        ...
        from dtcc_core.io.data.overpass import query_overpass_with_failover
        try:
            response = requests.get('https://overpass-api.de/api/interpreter', params={'data':query},
                headers={'User-Agent':'MR-Studio/1.0 (Swedish urban research; DTCC location preparation)', 'Accept':'application/json'}, timeout=(8,65))
            response.raise_for_status()
            data = response.json()
        except (requests.RequestException, ValueError):
            data = query_overpass_with_failover(query, read_timeout=65)
        if data.get('remark') or not isinstance(data.get('elements'), list):
            raise ValueError('Incomplete water source response')

        from studio.osm_water import water_features
        features = water_features(data)
        ...
        raise RuntimeError("Water source request failed") from e
...
def generate_dem(bounds, output_dir, pointcloud=None):
    ...
    raster = dtcc.build_terrain_raster(
        pointcloud, cell_size=CELL_SIZE, radius=3, ground_only=True
    )

    out_path = output_dir / "clipped_dem.geotiff.tif"
    dtcc.save_raster(raster, str(out_path))
    # DTCC terrain coordinates are SWEREF 99 TM, but its writer can omit CRS.
    import rasterio
    with rasterio.open(out_path, "r+") as dataset:
        if dataset.crs is None:
            dataset.crs = "EPSG:3006"
    log(f"  Saved {out_path} ({raster.width}x{raster.height}, "
        f"cell_size={raster.cell_size})")

    # Keep the terrain pristine. Stormwater barriers are a separate product.
    return pointcloud, raster


def build_display_mesh(buildings, raster):
    """Triangulate terrain and LOD1 buildings for display, without global unions.

    Components retain their source coordinates. The combined surface is not a
    watertight simulation/printing solid; footprints and DEM remain authoritative
    for the app's other analysis layers.
    """
    from dtcc_core.builder.meshing import merge_meshes
    meshes = [dtcc.build_terrain_surface_mesh(raster, max_mesh_size=10, smoothing=0)]
    for index, building in enumerate(buildings):
        if building.lod1 is None:
            raise ValueError(f"Building {building.id} has no LOD1 surface")
        mesh = building.lod1.mesh()
        if len(mesh.faces) == 0:
            raise ValueError(f"Building {building.id} produced an empty display mesh")
        meshes.append(mesh)
        if (index + 1) % 500 == 0:
            log(f"  Display mesh: {index + 1}/{len(buildings)} buildings")
    result = merge_meshes(meshes, weld=False)
    log(f"  Display mesh complete: {len(buildings)} buildings, {len(result.faces)} triangles")
    return result


def generate_mesh(bounds, output_dir, buildings=None, pointcloud=None, raster=None, mode="conditioned"):
    """Build a LOD1 city mesh and save as STL."""
    if mode not in ("conditioned", "display"):
    ...
    buildings = dtcc.build_lod1_buildings(buildings)
    if buildings and not any(building.lod1 is not None for building in buildings):
        raise ValueError("No building volumes could be generated")
    ...
    if mode == "display":
        mesh = build_display_mesh(city.buildings, raster)
    else:
        mesh = dtcc.build_city_surface_mesh(city, lod=dtcc.GeometryType.LOD1)
...
def main():
    ...
    # Clear DTCC cache
    # Keep downloaded source cache for reproducible retries.
