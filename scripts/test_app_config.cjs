...
assert.equal(base.MR_CALIBRATION.current.zoom,16.2299329163497);
...
const state={interactive_map_lindholmen_selected_calibration:custom.id,interactive_map_lindholmen_calibrations:JSON.stringify([custom])};
...
assert.equal(load({...state,interactive_map_lindholmen_selected_calibration:'original'}).MR_CALIBRATION.current.zoom,base.MR_CALIBRATION.current.zoom);
assert.equal(load({interactive_map_lindholmen_default_calibration:JSON.stringify(custom)}).MR_CALIBRATION.current.zoom,15);
...
assert.equal(load({interactive_map_default_calibration:JSON.stringify(custom)}).MR_CALIBRATION.current.zoom,base.MR_CALIBRATION.current.zoom, 'Campus calibration must not leak into Lindholmen');
