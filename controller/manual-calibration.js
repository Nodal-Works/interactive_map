...
function readCalibrationHistory() {
    try {
        return [ORIGINAL_CALIBRATION, ...JSON.parse(localStorage.getItem((window.APP_CONFIG.calibration.storagePrefix + 'calibrations')) || '[]')];
    } catch (error) {
...
function renderCalibrationHistory(container) {
    if (!container) return;
    const selectedId = localStorage.getItem((window.APP_CONFIG.calibration.storagePrefix + 'selected_calibration')) || 'original';
...
function renderManualCalibration(dashboardTitle, legendTitle, dashboardContent, legendContent) {
    ...
    try {
        const selectedId = localStorage.getItem((window.APP_CONFIG.calibration.storagePrefix + 'selected_calibration'));
        const selected = readCalibrationHistory().find(item => item.id === selectedId);
        const calibration = selected || JSON.parse(localStorage.getItem((window.APP_CONFIG.calibration.storagePrefix + 'default_calibration')) || 'null');
